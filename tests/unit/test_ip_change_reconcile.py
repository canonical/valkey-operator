...
The address-change reconcile must not depend on ``config-changed`` firing at the exact
moment Juju's network binding reports the new address. If that single window is missed,
a later reconcile (``update-status``) has to converge the unit — otherwise the unit keeps
serving a certificate whose SANs still carry the old IP.
"""
...
OLD_IP = "127.0.1.1"
# must match the autouse `mock_bind_address` fixture in conftest.py: it is what the unit
# now binds, while the databag and the on-disk certificate still carry OLD_IP
NEW_IP = "127.1.1.1"
...
def _reconcile_env() -> ExitStack:
    ...
    patches = (
        ...
        # `exec` here stands in for `openssl x509 -ext subjectAltName`: the SANs currently
        # on disk still carry OLD_IP, while the unit now binds NEW_IP.
        patch(
...
def test_update_status_refreshes_client_certificate_when_ip_changed(cloud_spec_vm):
    ...
    Reproduces the CI failure: the unit's address changed, but `config-changed` ran while
    the binding still reported the old address, so nothing requested a new certificate.
    """
    ...
    # A provider signs the cert when client TLS is on, so the charm must ask for a new CSR
    # rather than regenerate a self-signed one.
...
def test_update_status_regenerates_self_signed_certificate_when_ip_changed(cloud_spec_vm):
    ...
    # the reconcile completed, so the databag marker advances to the new address
...
def test_update_status_survives_certificate_read_failure(cloud_spec_vm):
    ...
    # nothing to defer against: the next interval retries anyway, so a backlog would only
    # replay the same work on every later hook
