...
def test_peer_tls_relation_created_no_client_tls(cloud_spec):
    ...
    assert status_is(state_out, TLSStatuses.MISSING_CLIENT_TLS.value)
...
def test_peer_tls_relation_created(cloud_spec):
    ...
    status_peer_relation = testing.PeerRelation(id=2, endpoint=STATUS_PEERS_RELATION)
    peer_tls_relation = testing.Relation(id=3, endpoint=PEER_TLS_RELATION_NAME)
    client_tls_relation = testing.Relation(id=4, endpoint=CLIENT_TLS_RELATION_NAME)
    container = testing.Container(name=CONTAINER, can_connect=True)

    state_in = testing.State(
        leader=True,
        relations={peer_relation, status_peer_relation, peer_tls_relation, client_tls_relation},
        containers={container},
        model=testing.Model(name="my-vm-model", type="lxd", cloud_spec=cloud_spec),
    )
...
def test_peer_tls_relation_broken(cloud_spec):
    ...
    with (
        ...
        assert state_out.get_relation(1).local_unit_data.get("peer-cert-ready") == "false"
        assert state_out.get_relation(1).local_unit_data.get("tls-peer-state") == "no-tls"
