...
def test_network_cut_primary(  # noqa: C901
    ...
    if substrate == Substrate.VM:
        ...
        # The unit re-reads its address on update-status, because Juju's network binding can still
        # report the old IP while the post-restore config-changed runs; at the default 5 minute
        # interval that reconcile would land long after the retries below give up.
